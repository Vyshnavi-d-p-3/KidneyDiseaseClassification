...
class ConfigurationManager:
    ...
    def get_training_config(self) -> TrainingConfig:
        ...
        return TrainingConfig(
            ...
            params_use_class_weights=params.USE_CLASS_WEIGHTS,
            params_use_lr_scheduler=params.USE_LR_SCHEDULER,
            params_early_stopping_patience=params.EARLY_STOPPING_PATIENCE,
            params_dropout_rate=params.DROPOUT_RATE,
            params_label_format=params.LABEL_FORMAT
        )

    def get_evaluation_config(self) -> EvaluationConfig:
        return EvaluationConfig(
            path_of_model         = Path(self.config.training.trained_model_path),
            path_of_effnet_model  = Path(self.config.training_effnet.trained_model_path),
    ...
    def get_training_effnet_config(self) -> TrainingConfig:
        ...
        return TrainingConfig(
            ...
            updated_base_model_path=Path(self.config.prepare_effnet.updated_base_model_path),
            training_data=Path(training_data),
            params_epochs=params.EPOCHS,
            params_batch_size=params.BATCH_SIZE,
            ...
            params_early_stopping_patience=params.EARLY_STOPPING_PATIENCE,
            params_dropout_rate=params.DROPOUT_RATE,
            params_label_format=self.params.LABEL_FORMAT
        )

    def get_prepare_effnetb2_config(self) -> PrepareBaseModelConfig:
        cfg    = self.config.prepare_effnetb2
        params = self.params.EFFICIENTNETB2
        return PrepareBaseModelConfig(
            root_dir=Path(cfg.root_dir),
            base_model_path=Path(cfg.base_model_path),
            updated_base_model_path=Path(cfg.updated_base_model_path),
    ...
    def get_training_effnetb2_config(self) -> TrainingConfig:
        tc     = self.config.training_effnetb2
        params = self.params.EFFICIENTNETB2
        training_data = os.path.join(self.config.data_ingestion.unzip_dir, "kidney-ct-scan-image")
        create_directories([Path(tc.root_dir)])
        return TrainingConfig(
            root_dir=Path(tc.root_dir),
            trained_model_path=Path(tc.trained_model_path),
            updated_base_model_path=Path(self.config.prepare_effnetb2.updated_base_model_path),
            training_data=Path(training_data),
            params_epochs=params.EPOCHS,
            params_batch_size=params.BATCH_SIZE,
            params_is_augmentation=self.params.AUGMENTATION,
